...
private:
    ...
    unsigned short int seconds_ : 6; // declare a secounds with 6 bits.(0 to 63)
    unsigned short int minutes_ : 6; // declare a minutes with 6 bits. (0 to 63)
    unsigned int hours_ : 17;        // declare a hours with 17 bits.  (0 to 131071)

    //**********************************************************************//

    //******************* private static const members *********************//
    static const unsigned short int SIXTY = 60;   // define a 1 minutes as 60 seconds_
...
public:
    ...
    friend Time operator+(const unsigned long, const Time &time);
...
{
    this->seconds_ = (seconds < 0 || seconds > MAX_MIN_SEC) ? throw invalid_argument("secound max is 59") : seconds & MASK_MIN_SEC;
}

inline void Time::set_minutes(int minutes)
{
    this->minutes_ = (minutes < 0 || minutes > MAX_MIN_SEC) ? throw invalid_argument("minutes max is 59") : minutes & MASK_MIN_SEC;
}

inline void Time::set_hours(int hours)
{
    this->hours_ = (hours < 0 || hours > MAX_MIN_SEC) ? throw invalid_argument("hours max is 100,000") : hours & MASK_HOUR;
}
//*********************************** class methods ********************************************//
inline unsigned long Time::get_time_by_seconds() const
{
    return this->seconds_ + SIXTY * this->minutes_ + this->hours_ * SIXTY * SIXTY;
}
...
{
    *this += time.get_time_by_seconds();
    return *this;
}

inline Time &Time::operator+=(const unsigned long seconds)
{
    this->add_seconds(seconds);                                      // setting by a function , in order to reduse calling copy constructor
    //*this=*this+seconds;                                           // using copy construcotr + operator + that return a new object from type Time
    return *this;
}

inline Time Time::operator+(const Time &time1) const
{
    return Time(this->get_time_by_seconds() + time1.get_time_by_seconds());        // create a new object of time that have the sum of seconds
}
