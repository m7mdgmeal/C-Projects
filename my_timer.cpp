...
void Timer::increse_clock(unsigned int seconds, string time_type)
{
    ...
    {
        if (!time_type.compare("m") && !time_type.compare("M") && !time_type.compare("Minute") && !time_type.compare("minute") && !time_type.compare("min") && !time_type.compare("Min"))
            this->clock += ONE_MINUTES;
